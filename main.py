...
def remove_invalid_choices(string, expansions, strings):
    # Remove R_j of letters that do not appear into any of the strings t_i
    letters = set()
    for s in strings:
...
def main():
    ...
    expansions_pruned = {key: value[:] for key, value in expansions.items()}

    solvable = remove_invalid_choices(string, expansions_pruned, strings)

    if expansions_pruned and solvable:
        answer =  run_brute_force(string, expansions_pruned, strings)
    else:
        answer = None
    
    if expansions_pruned == {}:
        answer = {}

    if answer is not None:
        print_answer(answer, expansions)
    else:
        print_no()
